...
namespace App\GameProvider\BetConstruct;
...
/**
 * Provider Class for BetConstruct
 */
class BetConstructProvider implements GameProviderInterface
{
    /**
     * BetConstruct game code definition
     */
    const ICORE_GAME_PROVIDER = 'bc';
    ...
    public function init(RequestInterface $request, ResponseInterface $response)
    {
        ...
        try {
            ...
            if ($isLogin && $config) {
                ...
                $this->scripts->attach([
                        'betconstruct' => [
                            'currencies' => $config['betconstruct_currency']
                        ]
                ]);
            }
            $this->scripts->attach([
                        'game_provider' => [
                            'betconstruct_virtuals' => [ 'countries' => $config['betconstruct_country']
                            ]
                        ]
                ], true);
        } catch (\Exception $e) {
