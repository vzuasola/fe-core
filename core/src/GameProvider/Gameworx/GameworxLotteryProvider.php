...
/**
 * Provider Class for Gameworx
 */
class GameworxLotteryProvider implements GameProviderInterface
{
    /**
    ...
    public function init(RequestInterface $request, ResponseInterface $response)
    {
        ...
        try {
            ...
            if ($isLogin && $config) {
                ...
                $this->scripts->attach([
                        'gameworx_lottery' => [
                            'currencies' => $config['gameworx_lottery_currency']
                        ]
                ]);
            }
            $this->scripts->attach([
                        'game_provider' => [
                            'gameworx_lottery' => [ 'countries' => $config['gameworx_lottery_country']
                            ]
                        ]
                ], true);
        } catch (\Exception $e) {
