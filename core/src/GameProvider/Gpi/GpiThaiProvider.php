...
/**
 * Provider Class for GpiThai Gaming
 */
class GpiThaiProvider implements GameProviderInterface
{
    ...
    public function init(RequestInterface $request, ResponseInterface $response)
    {
        ...
        try {
            ...
            if ($isLogin && $config) {
                ...
                $this->scripts->attach([
                    'gpi_thai' => [
                        'currencies' => $config['gpi_thai_lottey_currency'],
                        'languages' => $config['gpi_thai_lottey_language_mapping']
                    ]
                ]);
            }
                $this->scripts->attach([
                        'game_provider' => [
                            'gpi_thai' => [ 'countries' => $config['gpi_thai_lottey_country']
                            ]
                        ]
                ], true);
        } catch (\Exception $e) {
