...
/**
 * Provider Class for GpiKeno Gaming
 */
class GpiKenoProvider implements GameProviderInterface
{
    ...
    public function init(RequestInterface $request, ResponseInterface $response)
    {
        ...
        try {
            ...
            if ($isLogin && $config) {
                ...
                $this->scripts->attach([
                    'gpi_keno' => [
                        'currencies' => $config['gpi_keno_currency'],
                        'languages' => $config['gpi_keno_language_mapping']
                    ]
                ]);
            }
            ...
            $this->scripts->attach([
                        'game_provider' => [
                            'gpi_keno' => [ 'countries' => $config['gpi_keno_country']
                            ]
                        ]
                ], true);
        } catch (\Exception $e) {
