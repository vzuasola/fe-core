 ...
 *
 */
class MobileCashierToken implements TokenInterface
{
    private $player;
    private $config;
    private $parser;
    ...
    public function getToken($options)
    {
        ...
        if (!empty($headerConfigs['mcashier_mapping'])) {
            $mapping = Config::parseMultidimensional($headerConfigs['mcashier_mapping']);
        ...
        if (empty($link)) {
            $link = $headerConfigs['default_mcashier_link'] ?? null;
        }
