...
export default function modal(options) {
        ...
        utility.addEventListener(bodyTag, "click", function (evt) {
            ...
            utility.forEachElement(options.selector, function (elem) {
                if (target === elem) {
                    ...
                    if (modalWindow && !utility.hasClass(modalWindow, 'modal-active')) {
    ...
    function modalScrollBot(modalWindow, $modalScrollable) {
        setTimeout(function () {
            ...
            if (detectIE() === 8 || detectIE() === 9) {
                $this.centerModalContent(modalWindow);
            }
        }, 1);
    }
    ...
    // event listener for custom modal e.g. Dafa connect lightbox
    utility.addEventListener(document, "modal_init", modalOverlay);
