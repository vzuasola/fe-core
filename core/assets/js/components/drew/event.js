...
export default (function () {
    ...
    Event.listen = function (el, eventName, handler) {
        return Event.addEventListener(el, eventName, function (event) {
            var e = event || window.event;
            var target = e.target || e.srcElement;

            handler(e, target, event.customData);
        });
    };
    ...
    Event.eventType = function () {
        return (('ontouchstart' in window) || window.DocumentTouch && document instanceof DocumentTouch) ? "touchend" : "click";
    };
