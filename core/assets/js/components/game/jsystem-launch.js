...
/**
 * JSystem game launching
 */
function JSystemLaunch() {
    var requestFlag = 0,
        key = 'jsystem';
    ...
    this.launch = function (options) {
        if (!requestFlag) {
            ...
            xhr({
                ...
                if (response.gameurl) {
                    Console.log("JSystem::launch - Feth game  success");
                    if (typeof options.onSuccess === 'function') {
                        options.onSuccess.apply(null, [response]);
                    }
                } else {
                    Console.log("JSystem::launch - Feth game failed");
                    if (typeof options.onFail === 'function') {
                        options.onFail.apply(null, [options.providerProduct, response]);
                    }
                }
            }).fail(function (err, msg) {
                Console.log("JSystem::launch - Feth game failed", err, msg);
                if (typeof options.onFail === 'function') {
...
JSystemLaunch.prototype = GameLaunch.prototype;

export default JSystemLaunch;
