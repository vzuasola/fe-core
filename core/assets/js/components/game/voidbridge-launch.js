...
/**
 * Voidbridge game launching
 */
function VoidbridgeLaunch() {
    var requestFlag = 0,
        key = 'voidbridge';
    ...
    this.launch = function (options) {
        if (!requestFlag) {
            ...
            xhr({
                ...
                if (response.gameurl) {
                    Console.log("Voidbridge::launch - Feth game  success");
                    if (typeof options.onSuccess === 'function') {
                        options.onSuccess.apply(null, [response]);
                    }
                } else {
                    Console.log("Voidbridge::launch - Feth game failed");
                    if (typeof options.onFail === 'function') {
                        options.onFail.apply(null, [options.providerProduct, response]);
                    }
                }
            }).fail(function (err, msg) {
                Console.log("Voidbridge::launch - Feth game failed", err, msg);
                if (typeof options.onFail === 'function') {
...
VoidbridgeLaunch.prototype = GameLaunch.prototype;

export default VoidbridgeLaunch;
