...
function WACLaunch() {
    ...
    this.launch = function (options) {
        if (!requestFlag) {
            ...
            var endpoint = 'ajax/game/url/wac/' + (options.gamecode || ''),
                data = {
                    languageCode: gameLaunchUtil.getIcoreLanguageCode(key),
                };

            if (options.isLobby === true) {
                endpoint = 'ajax/game/lobby/wac';
            ...
            xhr({
                ...
                if (response.gameurl) {
                    Console.log("WAC::launch - Feth game  success");
                    if (typeof options.onSuccess === 'function') {
                        options.onSuccess.apply(null, [response]);
                    }
                } else {
                    Console.log("WAC::launch - Feth game failed");
                    if (typeof options.onFail === 'function') {
                        options.onFail.apply(null, ['']);
                    }
                }
            }).fail(function (err, msg) {
                Console.log("WAC::launch - Feth game failed", err, msg);
                if (typeof options.onFail === 'function') {
...
WACLaunch.prototype = GameLaunch.prototype;

export default WACLaunch;
