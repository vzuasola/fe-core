...
/**
 * Kaiyuan (KY) Gaming game launching
 */
function KYGamingLaunch() {
    var requestFlag = 0,
        key = 'ky_gaming';
    ...
    this.launch = function (options) {
        if (!requestFlag) {
            ...
            xhr({
                url: utility.url('ajax/game/url/ky-gaming/' + (options.gamecode || '')),
                method: 'get',
                type: 'json',
                data: data
            }).then(function (response) {
                if (response.gameurl) {
                    Console.log("KY Game::launch - Feth game success");
                    if (typeof options.onSuccess === 'function') {
                        options.onSuccess.apply(null, [response]);
                    }
                } else {
                    Console.log("KY Game::launch - Feth game failed");
                    if (typeof options.onFail === 'function') {
                        options.onFail.apply(null, [options.providerProduct, response]);
                    }
                }
            }).fail(function (err, msg) {
                Console.log("KY Game::launch - Feth game failed", err, msg);
                if (typeof options.onFail === 'function') {
...
KYGamingLaunch.prototype = GameLaunch.prototype;

export default KYGamingLaunch;
