...
/**
 * Thai specific game launching
 */
function GpiThaiLauncher() {
    ...
    function launchGame() {
        var loaderUrl = utility.url('/game/loader'),
            uri = utility.url('/game/gpithai/lobby'),
            windowObject = '';

        windowObject = PopupWindow(loaderUrl, 'GPIThaiGame', {width: 1050, height: 600});
        setTimeout(function () {
    ...
    function isCurrencySupported() {
        ...
        if (currency) {
            currencies = app.settings.gpi_pk10.currencies.split("\r\n");
...
GpiThaiLauncher.prototype = GameLaunch.prototype;

export default GpiThaiLauncher;
