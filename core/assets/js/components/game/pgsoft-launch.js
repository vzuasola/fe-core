...
/**
 * Pocket Games Soft game launching
 */
function PGSoftLaunch() {
    var requestFlag = 0,
        key = 'pgsoft';
    ...
    this.launch = function (options) {
        if (!requestFlag) {
            ...
            var endpoint = 'ajax/game/url/pgsoft/' + (options.gamecode || '');
            var data = {
            ...
            xhr({
                ...
                if (response.gameurl) {
                    Console.log("PGSoft Game::launch - Feth game success");
                    if (typeof options.onSuccess === 'function') {
                        options.onSuccess.apply(null, [response]);
                    }
                } else {
                    Console.log("PGSoft Game::launch - Feth game failed");
                    if (typeof options.onFail === 'function') {
                        options.onFail.apply(null, [options.providerProduct, response]);
                    }
                }
            }).fail(function (err, msg) {
                Console.log("PGSoft Game::launch - Feth game failed", err, msg);
                if (typeof options.onFail === 'function') {
...
PGSoftLaunch.prototype = GameLaunch.prototype;

export default PGSoftLaunch;
