...
function GpiSodeLauncher() {
    ...
    function launchGame() {
        var loaderUrl = utility.url('/game/loader'),
            uri = utility.url('/game/gpisode/lobby'),
            windowObject = '';

        windowObject = PopupWindow(loaderUrl, 'GPISodeGame', {width: 1050, height: 600});
        setTimeout(function () {
    ...
    function isCurrencySupported() {
        ...
        if (currency) {
            currencies = app.settings.gpi_sode.currencies.split("\r\n");
...
GpiSodeLauncher.prototype = GameLaunch.prototype;

export default GpiSodeLauncher;
