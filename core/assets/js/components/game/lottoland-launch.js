...
/**
 * Lottoland specific game launching
 */
function LottolandLauncher() {
    ...
    function launchGame() {
        var loaderUrl = utility.url('/game/loader'),
            uri = utility.url('game/lottoland/lobby'),
            windowObject = '';

        windowObject = PopupWindow(loaderUrl, 'Lottoland', {width: 1380, height: 800});
        setTimeout(function () {
    ...
    function isCurrencySupported() {
        ...
        if (currency) {
            currencies = app.settings.lottoland.currencies.split("\r\n");
...
LottolandLauncher.prototype = GameLaunch.prototype;

export default LottolandLauncher;
