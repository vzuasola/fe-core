...
/**
 * Ruby Play game launching
 */
function RubyPlayLaunch() {
    var requestFlag = 0,
        key = 'ruby_play';
    ...
    this.launch = function (options) {
        if (!requestFlag) {
            ...
            var endpoint = 'ajax/game/url/ruby-play/' + (options.gamecode || ''),
                data = {
                    languageCode: gameLaunchUtil.getIcoreLanguageCode(key),
                };

            if (options.isLobby === true) {
                endpoint = 'ajax/game/lobby/ruby-play';
            ...
            xhr({
                ...
                if (response.gameurl) {
                    Console.log("Ruby Play::launch - Feth game  success");
                    if (typeof options.onSuccess === 'function') {
                        options.onSuccess.apply(null, [response]);
                    }
                } else {
                    Console.log("Ruby Play::launch - Feth game failed");
                    if (typeof options.onFail === 'function') {
                        options.onFail.apply(null, [options.providerProduct, response]);
                    }
                }
            }).fail(function (err, msg) {
                Console.log("Ruby Play::launch - Feth game failed", err, msg);
                if (typeof options.onFail === 'function') {
...
RubyPlayLaunch.prototype = GameLaunch.prototype;

export default RubyPlayLaunch;
