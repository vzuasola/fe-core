...
/**
 * Evo Gaming game launching
 */
function EvoGamingLaunch() {
    var requestFlag = 0,
        key = 'evo_gaming';
    ...
    this.launch = function (options) {
        if (!requestFlag) {
            ...
            var endpoint = 'ajax/game/url/evo-gaming/' + (options.gamecode || ''),
                data = {
                    languageCode: gameLaunchUtil.getIcoreLanguageCode(key)
                };

            if (options.isLobby === true) {
                endpoint = 'ajax/game/lobby/evo-gaming';
            }
            ...
            xhr({
                ...
                if (response.gameurl) {
                    Console.log("Evo Gaming::launch - Feth game  success");
                    if (typeof options.onSuccess === 'function') {
                        options.onSuccess.apply(null, [response]);
                    }
                } else {
                    Console.log("Evo Gaming::launch - Feth game failed");
                    if (typeof options.onFail === 'function') {
                        options.onFail.apply(null, [options.providerProduct, response]);
                    }
                }
            }).fail(function (err, msg) {
                Console.log("Evo Gaming::launch - Feth game failed", err, msg);
                if (typeof options.onFail === 'function') {
...
EvoGamingLaunch.prototype = GameLaunch.prototype;

export default EvoGamingLaunch;
