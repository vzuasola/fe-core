...
/**
 * GoldenRace Virtual specific game launching
 */
function GoldenraceVirtualsLauncher() {
    ...
    function launchGame() {
        var loaderUrl = utility.url('/game/loader'),
            uri = utility.url('/launch/goldenrace'),
            windowObject = '';

        windowObject = PopupWindow(loaderUrl, 'GoldenRaceVirtuals', {width: 1380, height: 800});
        setTimeout(function () {
    ...
    function isCurrencySupported() {
        ...
        if (currency) {
            currencies = app.settings.golden_race.currencies.split("\r\n");
...
GoldenraceVirtualsLauncher.prototype = GameLaunch.prototype;

export default GoldenraceVirtualsLauncher;
