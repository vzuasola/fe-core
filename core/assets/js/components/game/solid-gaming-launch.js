...
/**
 * Solid gaming game launching
 */
function SolidGamingLaunch() {
    var requestFlag = 0,
        key = 'solid_gaming';
    ...
    this.launch = function (options) {
        if (!requestFlag) {
            ...
            xhr({
                url: utility.url('ajax/game/url/solid-gaming/' + (options.gamecode || '')),
                method: 'get',
                type: 'json',
                data: data
            }).then(function (response) {
                if (response.gameurl) {
                    Console.log("Solid Gaming::launch - Feth game success");
                    if (typeof options.onSuccess === 'function') {
                        options.onSuccess.apply(null, [response]);
                    }
                } else {
                    Console.log("Solid Gaming::launch - Feth game failed");
                    if (typeof options.onFail === 'function') {
                        options.onFail.apply(null, ['']);
                    }
                }
            }).fail(function (err, msg) {
                Console.log("Solid Gaming::launch - Feth game failed", err, msg);
                if (typeof options.onFail === 'function') {
...
SolidGamingLaunch.prototype = GameLaunch.prototype;

export default SolidGamingLaunch;
