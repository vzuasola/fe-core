...
/**
 * GlobalBet Virtual specific game launching
 */
function GlobalbetVirtualsLauncher() {
    ...
    function launchGame() {
        var loaderUrl = utility.url('/game/loader'),
            uri = utility.url('/launch/globalbet'),
            windowObject = '';

        windowObject = PopupWindow(loaderUrl, 'GlobalBetVirtuals', {width: 1380, height: 800});
        setTimeout(function () {
    ...
    function isCurrencySupported() {
        ...
        if (currency) {
            currencies = app.settings.globalbet_virtual_sports.currencies.split("\r\n");
...
GlobalbetVirtualsLauncher.prototype = GameLaunch.prototype;

export default GlobalbetVirtualsLauncher;
