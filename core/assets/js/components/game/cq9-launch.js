...
function CQ9Launch() {
    ...
    this.launch = function (options) {
        if (!requestFlag) {
            ...
            xhr({
                url: utility.url('ajax/game/url/cq9/' + (options.gamecode || '')),
                method: 'get',
                type: 'json',
                data: data
            }).then(function (response) {
                if (response.gameurl) {
                    Console.log("CQ9 Game::launch - Feth game success");
                    if (typeof options.onSuccess === 'function') {
                        options.onSuccess.apply(null, [response]);
                    }
                } else {
                    Console.log("CQ9 Game::launch - Feth game failed");
                    if (typeof options.onFail === 'function') {
                        options.onFail.apply(null, [options.providerProduct, response]);
                    }
                }
            }).fail(function (err, msg) {
                Console.log("CQ9 Game::launch - Feth game failed", err, msg);
                if (typeof options.onFail === 'function') {
...
CQ9Launch.prototype = GameLaunch.prototype;

export default CQ9Launch;
