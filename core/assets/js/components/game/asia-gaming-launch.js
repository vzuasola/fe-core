...
function AsiaGamingLaunch() {
    ...
    this.launch = function (options) {
        if (!requestFlag) {
            ...
            var endpoint = 'ajax/game/url/asia-gaming/' + (options.gamecode || ''),
                data = {
                    languageCode: gameLaunchUtil.getIcoreLanguageCode(key),
                };

            if (options.isLobby === true) {
                endpoint = 'ajax/game/lobby/asia-gaming';
            ...
            xhr({
                ...
                if (response.gameurl) {
                    Console.log("Asia Gaming::launch - Feth game  success");
                    if (typeof options.onSuccess === 'function') {
                        options.onSuccess.apply(null, [response]);
                    }
                } else {
                    Console.log("Asia Gaming::launch - Feth game failed");
                    if (typeof options.onFail === 'function') {
                        options.onFail.apply(null, [options.providerProduct, response]);
                    }
                }
            }).fail(function (err, msg) {
                Console.log("Asia Gaming::launch - Feth game failed", err, msg);
                if (typeof options.onFail === 'function') {
...
AsiaGamingLaunch.prototype = GameLaunch.prototype;

export default AsiaGamingLaunch;
