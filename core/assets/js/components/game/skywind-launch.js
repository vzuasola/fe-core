...
/**
 * Skywind game launching
 */
function SkywindLaunch() {
    var requestFlag = 0,
        codePath = '',
        key = 'skywind';
    ...
    this.launch = function (options) {
        if (!requestFlag) {
            ...
            var endpoint = 'ajax/game/url/skywind' + codePath,
                data = {
            ...
            if (options.isLobby === true) {
                endpoint = 'ajax/game/lobby/skywind';
            }
            ...
            // Override onFail method for voidbridge
            ...
            xhr({
                ...
                if (response.gameurl) {
                    Console.log("Skywind::launch - Feth game  success");
                    if (typeof options.onSuccess === 'function') {
                        options.onSuccess.apply(null, [response]);
                    }
                } else {
                    Console.log("Skywind::launch - Feth game failed");
                    if (typeof options.onFail === 'function') {
                        options.onFail.apply(null, [options.providerProduct, response]);
                    }
                }
            }).fail(function (err, msg) {
                Console.log("Skywind::launch - Feth game error", err, msg);
                if (typeof options.onFail === 'function') {
...
SkywindLaunch.prototype = GameLaunch.prototype;

export default SkywindLaunch;
