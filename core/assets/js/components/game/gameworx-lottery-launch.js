...
/**
 * Gameworx Lottery specific game launching
 */
function GameworxLotteryLauncher() {
    ...
    function launchGame() {
        var loaderUrl = utility.url('/game/loader'),
            uri = utility.url('/launch/gameworx-lottery'),
            windowObject = '';

        windowObject = PopupWindow(loaderUrl, 'GWX Lottery', {width: 988, height: 595});
        setTimeout(function () {
    ...
    function isCurrencySupported() {
        ...
        if (currency) {
            currencies = app.settings.gameworx_lottery.currencies.split("\r\n");
...
GameworxLotteryLauncher.prototype = GameLaunch.prototype;

export default GameworxLotteryLauncher;
