...
/**
 * Fun Gaming game launching
 */
function FunGamingLaunch() {
    var requestFlag = 0,
        key = 'fun_gaming';
    ...
    this.launch = function (options) {
        if (!requestFlag) {
            ...
            xhr({
                url: utility.url('ajax/game/url/fun-gaming/' + (options.gamecode || '')),
                method: 'get',
                type: 'json',
                data: data
            }).then(function (response) {
                if (response.gameurl) {
                    Console.log("Fun Gaming::launch - Feth game  success");
                    if (typeof options.onSuccess === 'function') {
                        options.onSuccess.apply(null, [response]);
                    }
                } else {
                    Console.log("Fun Gaming::launch - Feth game failed");
                    if (typeof options.onFail === 'function') {
                        options.onFail.apply(null, [options.providerProduct, response]);
                    }
                }
            }).fail(function (err, msg) {
                Console.log("Fun Gaming::launch - Feth game failed", err, msg);
                if (typeof options.onFail === 'function') {
...
FunGamingLaunch.prototype = GameLaunch.prototype;

export default FunGamingLaunch;
